        ...
        populateData: function () {
            ...
            for (i = start; i < end; i++) {
                body.innerHTML += addRow(
                    ...
                    pagin.list[i].BasePurchase,
                    pagin.list[i].StartDate,
                    pagin.list[i].EndDate,
...
function addRow(counter, ID, Name, Discount, BasePurchase, StartDate, EndDate, Description) {
    var s = "";
    s = "<tr>";
    s += "<td title='" + ID + "'>" + counter + "</td>";
    s += "<td>" + Name + "</td>";
    s += "<td>" + Discount + "</td>";
    s += "<td>" + BasePurchase + "</td>";
    s += "<td>" + StartDate + "</td>";
    s += "<td>" + EndDate + "</td>";
    s += "<td>" + Description + "</td>"; 
    var day = new Date();
    var today = new Date(dateFormat(day));
    var endDate = new Date(dateInPut(String(EndDate)));
   
    // Biểu thức dưới khi endDate="Đến nay" cũng ra đúng yêu cầu nên ta k xét điều kiện tường minh nữa.
    if (!(endDate < today)) {   // so sánh ngày kết thúc của km nếu lớn hơn hoặc bằng ngày hiện tại, thì là km hiện hành, ngược lại là các km đã kết thúc

        s += "<td title='currentPro'> <button title='Edit' type='button' class='btn btn-info' data-toggle='modal' data-target='#myModal' onclick='loadInFo(this);'><span ><i class='glyphicon glyphicon-edit'></i></span></button>  <button title='Delete' type='button' class='btn btn-info' style='color:red' data-toggle='modal' data-target='#deleteModal' onclick='deletePromotion(this);'><span class='glyphicon glyphicon-remove-circle'></span></button> </td>";

    } else {

        s += "<td> <button title='Clone' type='button' class='btn btn-info' data-toggle='modal' data-target='#myModal' onclick='loadInFo(this);'><span><i class='fa fa-clone'></i></span></button> </td>";
    }
...
function searchPagination() {
    ...
    if (text !== '') {
        lst.forEach(function (item) {
            if ((item.Name.removeMark().search(text) !== -1)
                || (item.Discount.search(text) !== -1)
                || (item.BasePurchase.search(text) !== -1)
                || (item.StartDate.search(text) !== -1)
                || (item.EndDate.search(text) !== -1)
