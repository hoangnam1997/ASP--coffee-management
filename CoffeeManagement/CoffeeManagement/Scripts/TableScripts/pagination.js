        ...
        populateData: function () {
            ...
            for (i = start; i < end; i++) {
                body.innerHTML += addRow(
                    ...
                    pagin.list[i].ID,
                    pagin.list[i].Name,
                    pagin.list[i].GroupTableID,
                    pagin.list[i].GroupTableName,
...
function addRow(counter, TableID, CellName, CellGroupTableID, CellGroupTableName, CellDes) {
    var s = "";
    s = "<tr>";
    s += "<td title='" + TableID + "'>" + counter + "</td>";
    s += "<td>" + CellName + "</td>";
    ...
    if (text !== '') {
        lst.forEach(function (item) {
            if ((item.ID.toString().search(text) !== -1)
                ...
                || (item.GroupTableName.removeMark().search(text) !== -1)
